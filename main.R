...
skill_list <- c(
  ...
  "kotlin", "go", "php", "perl", "ruby", "swift", "objective-c", "r", "matlab",
  "vb\\.net", "bash", "shell scripting", "powershell",
  
  # Web Technologies
  "html", "html5", "css", "css3", "sass", "less", "xml", "json", "ajax", "rest", 
  "soap", "graphql", "websocket",
  
  # JavaScript Frameworks/Libraries
  "react", "reactjs", "angular", "angularjs", "vue", "vuejs", "node", "nodejs", 
  "express", "expressjs", "jquery", "backbone", "ember", "meteor", "next\\.js",
  "gatsby", "svelte", "polymer", "knockout", "extjs", "dojo",
  
  # Java Frameworks
  "spring", "spring boot", "spring mvc", "spring security", "hibernate", "jpa",
  "struts", "jsf", "ejb", "jsp", "servlet", "mybatis", "apache camel",
  
  # Python Frameworks
  "django", "flask", "fastapi", "tornado", "pyramid", "bottle",
  
  # PHP Frameworks
  "laravel", "symfony", "codeigniter", "zend", "cakephp",
  
  # Databases
  "sql", "mysql", "postgresql", "oracle", "sql server", "mongodb", "cassandra",
  "redis", "dynamodb", "couchdb", "hbase", "neo4j", "elasticsearch", "db2",
  "mariadb", "sqlite", "teradata",
  
  # Cloud Platforms
  "aws", "azure", "gcp", "google cloud", "ec2", "s3", "lambda", "cloudfront",
  "cloud foundry", "openshift", "heroku", "digitalocean",
  
  # DevOps/CI-CD
  "docker", "kubernetes", "jenkins", "gitlab", "circleci", "travis ci",
  "ansible", "puppet", "chef", "terraform", "vagrant", "bamboo",
  
  # Version Control
  "git", "github", "gitlab", "svn", "mercurial", "bitbucket", "clearcase",
  
  # Testing
  "junit", "testng", "selenium", "cucumber", "jest", "mocha", "jasmine",
  "pytest", "unittest", "mockito", "cypress", "protractor", "karma",
  
  # Build Tools
  "maven", "gradle", "ant", "webpack", "gulp", "grunt", "npm", "yarn",
  
  # Big Data
  "hadoop", "spark", "hive", "pig", "kafka", "storm", "flink", "hbase",
  "mapreduce", "yarn", "sqoop", "flume",
  
  # BI/Reporting
  "tableau", "power bi", "qlikview", "looker", "cognos", "ssrs", "crystal reports",
  "jasper reports", "pentaho", "microstrategy",
  
  # CMS/E-commerce
  "drupal", "wordpress", "joomla", "magento", "shopify", "woocommerce",
  "aem", "adobe experience manager", "sitecore",
  ...
  # Operating Systems
  "linux", "unix", "windows", "ubuntu", "centos", "red hat", "debian",
  "mac os", "solaris", "aix",
  
  # Tools
  "jira", "confluence", "trello", "slack", "ms office", "excel", "visio",
  "postman", "soapui", "splunk", "nagios", "grafana", "prometheus",
  ...
  # Other Technologies
  "etl", "informatica", "talend", "ssis", "ab initio", "data warehouse",
...
extract_skills <- function(text) {
  ...
  # Create binary flags for each skill
  binary_flags <- setNames(
    as.list(as.integer(skill_list %in% found)),
    paste0("skill_", gsub("\\\\\\+", "plus", gsub("#", "sharp", gsub("\\.", "_", gsub(" ", "_", skill_list)))))
  )
  
  list(
    skills_detected = if(length(found) > 0) paste(found, collapse = ", ") else NA,
    skill_count = length(found),
    binary = binary_flags
  )
}
...
calculate_seniority <- function(text) {
  ...
  years_sorted <- sort(as.numeric(year_nums))
  total_years <- max(years_sorted) - min(years_sorted)
  
  seniority_level <- case_when(
    total_years < 2 ~ "Junior",
    total_years < 5 ~ "Mid",
    total_years < 8 ~ "Senior",
    total_years >= 8 ~ "Lead",
    TRUE ~ "Unknown"
  )
...
df_seniority <- df_results %>%
  select(person_id, raw_text) %>%
  mutate(
    seniority_data = map(raw_text, calculate_seniority),
    total_years = map_dbl(seniority_data, ~ .x$total_years),
    seniority_level = map_chr(seniority_data, ~ .x$seniority_level)
  ) %>%
  select(person_id, total_years, seniority_level)
...
df_skills <- df_results %>%
  select(person_id, raw_text) %>%
  mutate(
    skill_data = map(raw_text, extract_skills),
    skills_detected = map_chr(skill_data, ~ .x$skills_detected %||% NA_character_),
    skill_count = map_int(skill_data, ~ .x$skill_count)
  ) %>%
...
cat("\nSeniority Distribution:\n")
print(table(PERSON$seniority_level))

# Skill statistics
cat("\nSkill Statistics:\n")
cat("Min skills:", min(PERSON$skill_count, na.rm = TRUE), "\n")
cat("Max skills:", max(PERSON$skill_count, na.rm = TRUE), "\n")
cat("Avg skills:", round(mean(PERSON$skill_count, na.rm = TRUE), 1), "\n")
